...
export default function OhsReportDocument({
  statRows,
  classification,
  targets,
  organisationName,
  ...
  const reportRows = useMemo(() => statRows.filter(isOhsScorecardRow), [statRows]);
  const scorecardRows = useMemo(
    () => buildScorecardRows(statRows, targets),
    [statRows, targets]
  );
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", border: "1px solid #d1d5db" }}>
            <thead>
              <tr>
                ...
                <th style={{ ...thDark, width: 80, borderRight: "none" }}>Status</th>
              </tr>
            </thead>
            <tbody>
              {scorecardRows.map((row, index) => {
                const { style: v, status: statusKey } = scorecardVarianceStatus(
                  row.target,
                  row.actual,
                  row.lowerBetter
                );
