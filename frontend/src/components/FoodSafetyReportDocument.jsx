...
export default function FoodSafetyReportDocument({
  statRows,
  incidents,
  targets,
  organisationName,
  ...
  const reportRows = useMemo(() => statRows.filter(isFsStatRow), [statRows]);
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", border: "1px solid #d1d5db" }}>
          ...
          <tbody>
            {scorecardRows.map((row, index) => {
              const { style: v, status: statusKey } = fsScorecardVarianceStatus(
                row.target,
                row.actual,
                row.lowerBetter
              );
