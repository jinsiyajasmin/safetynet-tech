...
import { Leaf, Plus, Recycle, Trash2 } from "lucide-react";
...
export default function EnvironmentalMonthlyStatistics({ rows, onRowsChange, waste, onWasteChange }) {
  ...
  const updateWaste = (key, value) => {
    onWasteChange((prev) => ({ ...prev, [key]: value }));
  };

  return (
    <Box sx={{ mb: 3 }}>
      <SectionBanner icon={Leaf} title="SECTION 1 — ENVIRONMENTAL STATISTICS" />
        ...
        >
          <thead>
            <tr>
              ...
              <th style={{ ...thDark, textAlign: "left", minWidth: 280 }}>Indicator</th>
              {ENV_MONTHS.map((m) => (
      ...
      <SectionBanner
        icon={Recycle}
        title="WASTE GENERATION & DISPOSAL — ANNUAL SNAPSHOT (KGs)"
        color="#111827"
      />
              ...
              {ENV_WASTE_CATEGORIES.map((cat) => (
                <td key={cat.key} style={tdBase}>
                  <input
                    type="text"
                    value={waste[cat.key] || ""}
                    onChange={(e) => updateWaste(cat.key, e.target.value)}
                    placeholder="0"
                    style={{
                      width: "100%",
                      border: "none",
      ...
      <p style={{ margin: "10px 0 0", fontSize: 12, color: T.inkFaint }}>
        Enter monthly environmental values above. Renewable energy % on the scorecard is calculated
        from renewable and non-renewable YTD totals. Waste weights are entered in the annual snapshot.
      </p>
    </Box>
  );
}
